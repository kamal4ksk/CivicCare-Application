...
/**
 * Component 115 - Edit Article Modal
 *
 * Props:
 *  - isOpen: boolean
 *  - article: { id, title, category, description, imageUrl } | null
 *  - onClose: () => void
 *  - onSubmit: (id, updates) => void
 */
export function EditArticleModal({ isOpen, article, onClose, onSubmit }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!article) return;
    onSubmit(article.id, { title, category, description, imageUrl });
    onClose();
  };
